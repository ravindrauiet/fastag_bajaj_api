...
const CHANNEL_old = 'CBPL'; // UAT Channel
const AGENT_ID_old = '70003'; // UAT Agent ID
const CHANNEL = 'TMSQ'; // Production Channel
const AGENT_ID = '70043'; // Production Agent ID
...
const bajajApi = {
  ...
  replaceFastag: async (payload) => {
    try {
      ...
      const response = await axios.post(
        `${PROD_URL}/ftAggregatorService/v2/replaceFastag`,
        encryptedData,
        {
          headers: {
            'Content-Type': 'application/json',
  ...
  checkBajajAppStatus: async (mobileNo) => {
    try {
      ...
      console.log('API_SUBSCRIPTION_KEY', API_SUBSCRIPTION_KEY);
      console.log('ENCRYPTION_KEY', ENCRYPTION_KEY);
      ...
      const requestData = {
        mobNo: mobileNo,
        aggrChannel: CHANNEL, // Use PKPL as in the documentation, not CBPL
        agentId: AGENT_ID, // Use the agent ID from documentation
        udf1: "",
        udf2: "",
        udf3: "",
        udf4: "",
      ...
      // API call with correct endpoint and headers as per documentation
      const response = await axios.post(
        `${PROD_URL}/ftAggregatorService/v1/initDataCheck`, 
        encryptedData, 
        {
          headers: {
            'Content-Type': 'application/json',
      ...
      if (response.data) {
        ...
        return {
          ...
          appInstalled: parsedData.appInstalled === true
        };
      } else {
      ...
      console.error('Check Bajaj app status error:', error);
      return {
        response: {
          status: 'error',
          code: error.code || 'UNKNOWN_ERROR',
